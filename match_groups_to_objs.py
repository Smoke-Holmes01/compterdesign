...
def safe_hu_distance(mask_a: np.ndarray, mask_b: np.ndarray):
    cnts_a, _ = cv2.findContours((mask_a * 255).astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    cnts_b, _ = cv2.findContours((mask_b * 255).astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    if len(cnts_a) == 0 or len(cnts_b) == 0:
        return 1e6

    c_a = max(cnts_a, key=cv2.contourArea)
    c_b = max(cnts_b, key=cv2.contourArea)

    return cv2.matchShapes(c_a, c_b, cv2.CONTOURS_MATCH_I1, 0.0)
...
def match_one_image_to_one_view(img_mask, img_edge, view_mask, view_edge):
    img_mask = resize_binary(img_mask)
    img_edge = resize_binary(img_edge)
    view_mask = resize_binary(view_mask)
    view_edge = resize_binary(view_edge)

    shape_iou = compute_iou(img_mask, view_mask)
    edge_iou = compute_iou(img_edge, view_edge)
    hu_dist = safe_hu_distance(img_mask, view_mask)
...
def score_one_image_against_one_model(image_record, model_views):
    img_mask = load_binary(image_record["mask_path"])
    img_edge = load_binary(image_record["edge_path"])

    best_view = None
    for view in model_views:
        try:
            view_mask = load_binary(view["mask_path"])
            view_edge = load_binary(view["edge_path"])

            score_info = match_one_image_to_one_view(img_mask, img_edge, view_mask, view_edge)
            ...
            print(f"[WARN] view compare failed | {view.get('mask_path', '')} | {e}")
...
def build_score_matrix(preprocessed_root: str, render_root: str, output_dir: str):
    ...
    groups = load_group_meta(preprocessed_root)
    models = load_render_meta(render_root)
